...
PROCESS_AGENT_TAGS = AGENT_TAGS.union(
    {"clusterchecks", "fargateprocess", "orchestrator"}
)
...
def get_build_tags(include, exclude):
    ...
    for tag in unknown_include:
        print(
            f"Warning: unknown build tag '{tag}' was filtered out from included tags list."
        )
    ...
    for tag in unknown_exclude:
        print(
            f"Warning: unknown build tag '{tag}' was filtered out from excluded tags list."
        )
